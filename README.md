...
Read [EDITOR-GUIDE.md](EDITOR-GUIDE.md). All live learning content is in `content/`. Templates are in `templates/`. The JavaScript and HTML do not need changes for ordinary content edits.

- `content/course.md`: course title and description.
- `content/journeys/`: line names, descriptions, colors and checkpoint lengths.
- `content/terms/`: lesson text, flashcard front/back, examples and related terms.
- `content/questions/`: question prompts, choices, correct answers and explanations.
- `content/exam.md`: exam title, description, question count and passing score.
...
`index.html` is the working app. `original/AI Lingo.dc.html`, `original/terms.js`, and `original/ai-lingo-catalog.md` are preserved original references and are not live content sources. `original/support.js` is the original generated runtime, retained as supplied. `dist/` is generated and ignored by Git. The current map supports up to five numbered lines; adding more requires a layout change. Content can be added freely within those lines.
